...
// --- Poster and Backdrop ---
function TvBackdrop({ backdrop, title }: { backdrop: string, title: string }) {
...
function TvPoster({ poster, title }: { poster: string, title: string }) {
...
// --- Tv Details ---
function TvDetails({ movie }: { movie: any }) {
  ...
  try {
    ...
    // Try to find official trailer first, then teaser, then any video
  ...
  return (
    <div className="p-2 md:w-4/5 lg:w-1/2">
      <h1 className="text-xl md:text-4xl font-Inter-SM">{movie.name}</h1>
  ...
  return (
    <>
      <div className="text-white relative flex items-center justify-center lg:h-150">
        <TvBackdrop backdrop={backdrop} title={movie.title} />
        <div className="p-4 md:pl-8 md:pt-6 lg:flex lg:pt-0 lg:justify-center">
          <div className="flex justify-center p-2 md:justify-start lg:mr-8 mb-4">
            <TvPoster poster={poster} title={movie.title} />
          </div>
